    ...
    <main>
        ...
        <section class="relative overflow-hidden pt-12 pb-24 px-8 max-w-screen-2xl mx-auto">
            <div class="grid lg:grid-cols-2 gap-12 items-center">
                <div class="z-10">
                    <span
                        class="inline-block px-4 py-1.5 bg-tertiary-container text-on-tertiary-container rounded-full text-xs font-bold tracking-widest uppercase mb-6">Curated
                        2026 Edition</span>
                    <h1
                        class="font-headline text-6xl lg:text-7xl font-extrabold text-on-surface tracking-tighter leading-[1.05] mb-8">
                    ...
                    <p class="text-on-surface-variant text-xl max-w-lg mb-10 leading-relaxed">
                        Discover a hand-picked collection of premium nursery essentials, developmental toys, and
                        eco-conscious clothing designed for the modern family.
                    </p>
                    <div class="flex flex-wrap gap-4">
                        <a href="{{ route('category.desktop') }}"
                            class="bg-gradient-to-b from-primary-dim to-primary px-8 py-4 rounded-xl text-on-primary font-bold text-lg shadow-lg hover:brightness-110 transition-all flex items-center gap-2">
                            Explore Recommendations
                            <span class="material-symbols-outlined">arrow_forward</span>
                        </a>
                        <a href="#top-10"
                            class="bg-surface-container-high px-8 py-4 rounded-xl text-on-surface font-bold text-lg hover:bg-surface-container-highest transition-all">
                            View Top 10 List
                        </a>
                    </div>
                </div>
                <div class="relative">
                    ...
                    <div
                        class="absolute -bottom-10 -left-10 w-64 h-64 rounded-lg overflow-hidden editorial-shadow transform -rotate-6 hidden lg:block border-8 border-white">
                        <img class="w-full h-full object-cover"
                            src="https://lh3.googleusercontent.com/aida-public/AB6AXuCztnmsf5n-t6_osXe12foCy9XHPROKiUm1_WQ0uETajWYhO7_iWXKZ8BtaBmUT6iz_l_t-0UkEUmPhiO12lukGL0QIEwHeS7eZL77kGLUSEB6xf1l4btU5xIiB8oLOE7BXiw4FzjtBrjeg4sczE8F2We-HZEo4VQWwDY98QP4Pc4WNg3PVV8VWm5yiokupSGvK0upB5wus1deDi69kKwCqU2G7tkwCefLbOxr-SZCKLGVCJTIIAYap6MwwFtjSXKdJGY97H50WWA" />
                    </div>
                </div>
            </div>
        </section>
            ...
            <div class="max-w-screen-2xl mx-auto">
                <div class="flex justify-between items-end mb-16">
                    <div>
                        ...
                        <p class="text-on-surface-variant max-w-md">Every item in our playroom is selected for quality,
                            safety, and aesthetic appeal.</p>
                    </div>
                    <a class="text-primary font-bold flex items-center gap-1 hover:underline"
                        href="{{ route('category.desktop') }}">
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-6 h-[600px]">
                    @foreach($cats as $cat)
                        <div
                            class="{{ ($cat['large'] ?? false) ? 'md:col-span-2 md:row-span-2' : (($cat['span'] ?? false) ? 'md:col-span-2' : '') }} relative group cursor-pointer overflow-hidden rounded-lg">
                            <img class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105"
                                src="{{ $cat['img'] }}" />
                            <div
                                class="absolute inset-0 {{ $cat['color'] ?? 'bg-gradient-to-t from-black/60 via-transparent to-transparent' }} {{ isset($cat['color']) ? 'group-hover:bg-transparent transition-colors' : '' }}">
                            </div>
                            @if($cat['large'] ?? false)
                                <div class="absolute bottom-8 left-8 text-white">
                                    ...
                                    <span
                                        class="bg-white/20 backdrop-blur-md px-4 py-2 rounded-full text-xs font-bold uppercase tracking-wider">Browse
                                        {{ $cat['items'] }}</span>
                                </div>
                            @else
                                <div
                                    class="absolute {{ ($cat['span'] ?? false) ? 'inset-0 flex items-center justify-center' : 'bottom-6 left-6' }} text-white">
                                    <h3 class="font-headline {{ ($cat['span'] ?? false) ? 'text-2xl' : 'text-xl' }} font-bold">
                                        {{ $cat['label'] }}</h3>
                                </div>
                            @endif
                        </div>
        ...
        <section id="top-10" class="py-24 px-8 max-w-screen-2xl mx-auto">
            ...
            <div class="text-center mb-16">
                ...
                <p class="text-on-surface-variant max-w-2xl mx-auto italic text-lg">"Our rigorous testing process involves
                    child psychologists, safety experts, and real parents to bring you the definitive list."</p>
            </div>
        ...
        <section class="py-24 px-8 max-w-screen-xl mx-auto">
            <div class="bg-primary rounded-xl p-12 lg:p-20 text-center relative overflow-hidden">
                <div
                    class="absolute top-0 left-0 w-full h-full bg-[radial-gradient(circle_at_top_right,_var(--tw-gradient-stops))] from-primary-container/30 to-transparent">
                </div>
                <div class="relative z-10">
                    <h2 class="font-headline text-4xl font-bold text-on-primary mb-6 tracking-tight">Join the Inner Circle
                    </h2>
                    <p class="text-on-primary/80 text-lg mb-10 max-w-xl mx-auto">Receive early access to our seasonal
                        curated guides and exclusive discount codes from our partner brands.</p>
                    <form class="flex flex-col md:flex-row gap-4 max-w-lg mx-auto">
                        <input
                            class="flex-1 rounded-xl bg-white/10 border-white/20 text-white placeholder:text-white/60 focus:ring-primary-container focus:border-primary-container px-6 py-4 backdrop-blur-sm"
                            placeholder="Your email address" type="email" />
                        <button
                            class="bg-white text-primary font-bold px-8 py-4 rounded-xl hover:bg-surface-bright transition-all whitespace-nowrap">
                            Get Guides
                        </button>
                    </form>
...
@push('styles')
    <style>
        .editorial-shadow {
            box-shadow: 0 20px 50px -12px rgba(44, 47, 48, 0.06);
        }
    </style>
@endpush